...
# Optional: Push to remote after successful commit
$remotes = git remote
if ($null -ne $remotes) {
    Write-Host ""
    $doPush = Read-Host "Changes committed. Do you want to push to remote? (y/N)"
    if ($doPush -eq 'y' -or $doPush -eq 'Y') {
        Write-Host "[specify] Pushing to remote..." -ForegroundColor Cyan
        git push
    } else {
        Write-Host "[specify] Skip push." -ForegroundColor DarkGray
    }
}
